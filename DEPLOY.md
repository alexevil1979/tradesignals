...
`fetch_candles.php` загружает котировки **только с mainnet** `https://api.bybit.com` (как `example/bbb/bothour/bybit_fill_tables.php` и страница bybit.com): первичная история до 5000 баров страницами по 200 с `end`, далее — обновление последних N баров. Папка `example/` в Git не публикуется.

Если в БД уже лежат кривые свечи, один раз выполните:

```bash
/usr/local/php82/bin/php bin/reset_candles.php
```

Скрипт очистит `candles` по BTCUSDT и заново подтянет историю. Обычный cron после этого только догружает недостающее.
## 7. Обновление из Git
