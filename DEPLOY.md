...
`fetch_candles.php` сначала подгружает максимально доступную историю Bybit (пачками до 1000 свечей, до 50 страниц за запуск на интервал), затем помечает историю как полную и дальше догружает только недостающие новые бары. Повторяйте запуск, пока в выводе не будет «история полная» для всех интервалов `1`, `5`, `15`, `60`, `240`, `D`. Графики на Dashboard показывают последние 100 баров из уже сохранённой истории.
## 7. Обновление из Git
